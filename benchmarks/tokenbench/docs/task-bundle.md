...
work. This contract does not read a filesystem, open a CAS object, or execute a
toolchain or evaluator.
...
silently alias one digest under conflicting metadata. A later loader must still
verify referenced bytes against the CAS identity; v1 only authenticates and
validates the manifest description.
