...
Use repo-view for repository source navigation. Do not use git, rg, grep, sed, cat, nl, head, tail, find, ls, or direct file reads under the repository root. Do not run an arbitrary shell command, including tests or dependency and standard-library readers, unless a task-specific block below supplies that command verbatim. An unregistered command is treated as capable of reading repository input and invalidates the run. This protocol is part of the task and its mechanical validation; an answer produced after bypassing it is invalid.
