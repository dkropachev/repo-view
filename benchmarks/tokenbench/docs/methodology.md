...
Code tasks cannot contain prose fact/rubric items and `BlindPair` rejects them. They instead target a sealed `ObjectiveCodeQuality` boundary that will eventually bind authenticated per-arm patch/test outcome digests. A production Linux authority now creates and verifies each fresh bounded writable workspace, but no production objective-outcome constructor, patch capture, or code runner exists yet; current code-task quality must therefore remain explicitly missing rather than being routed through prose judges or caller-authored booleans.
