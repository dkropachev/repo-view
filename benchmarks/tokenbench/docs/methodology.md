...
Code tasks cannot contain prose fact/rubric items and `BlindPair` rejects them. They instead target a sealed `ObjectiveCodeQuality` boundary that will eventually bind authenticated per-arm patch/test outcome digests. A production Linux authority now creates and verifies each fresh bounded writable workspace and can freeze and capture a bounded deterministic patch outcome after execution. No production objective-outcome constructor, signed evidence edge, hidden evaluator, or code runner is connected yet; current code-task quality must therefore remain explicitly missing rather than being routed through prose judges or caller-authored booleans.
