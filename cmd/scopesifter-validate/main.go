...
func run(args []string) int {
	flags := flag.NewFlagSet("scopesifter-validate", flag.ContinueOnError)
...
func readSourceFiles(root string) ([]fileData, error) {
	...
	for _, extension := range navigator.SupportedExtensions() {
...
func compareLocationResults(got []navigator.Result, want []string) string {
...
func validateScopes(results []navigator.Result, files []fileData, symbol string) error {
...
func validateCode(results []navigator.Result, files []fileData) error {
	fileMap := mapFiles(files)
	for _, result := range results {
		file, ok := fileMap[result.Path]
		if !ok {
...
func resultLocation(result navigator.Result) string {
