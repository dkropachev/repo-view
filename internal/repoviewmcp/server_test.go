...
`)
	...
	cache := repoview.ChangedStateCache{
		SchemaVersion: repoview.ChangedStateSchemaVersion,
		BaseCommit:    base,
		HeadCommit:    head,
		HeadSubject:   "cache-only",
		ChangedFiles: []repoview.ChangedFileState{{
			Path: "demo.go", Status: "modified",
			Lines: []repoview.ChangedLineSpan{{Start: 3, End: 3}},
...
func TestServerManifestAndToolsAreReadOnly(t *testing.T) {
	...
	assertToolOutput(t, client, "changed", map[string]any{}, func() (repoview.ChangedResponse, error) {
		return view.Changed(repoview.Options{
			Base: fixture.base, Include: repoview.IncludeAll,
			Return: repoview.ReturnContext, Context: defaultContext,
			Limit: defaultLimit, MaxCodeLines: defaultMaxCodeLines,
			MaxPatchLines: defaultMaxPatchLines,
		})
	})
	assertToolOutput(t, client, "find", map[string]any{"symbol": "Helper"}, func() (repoview.FindResponse, error) {
		return view.Find("Helper", repoview.Options{
			Base: fixture.base, Include: repoview.IncludeBoth,
			Return: repoview.ReturnScope, Context: defaultContext,
			Limit: defaultLimit, MaxCodeLines: defaultMaxCodeLines,
			MaxPatchLines: defaultMaxPatchLines, NoComments: true, NoStrings: true,
		})
	})
	assertToolOutput(t, client, "inspect", map[string]any{"location": "demo.go:6"}, func() (repoview.InspectResponse, error) {
		return view.Inspect("demo.go:6", repoview.Options{
			Base: fixture.base, Include: repoview.IncludeScope,
			Return: repoview.ReturnScope, Context: defaultContext,
			Limit: defaultLimit, MaxCodeLines: defaultMaxCodeLines,
			MaxPatchLines: defaultMaxPatchLines, NoComments: true, NoStrings: true,
		})
	})
	assertToolOutput(t, client, "outline", map[string]any{"path": "demo.go"}, func() (repoview.OutlineResponse, error) {
		return view.Outline("demo.go", repoview.Options{
			Base: fixture.base, Include: repoview.IncludeDefs,
			Return: repoview.ReturnLine, Limit: defaultLimit,
...
func newFixture(t *testing.T) fixture {
	...
	gitRun(t, gitPath, root, "config", "user.email", "repo-view@example.test")
	gitRun(t, gitPath, root, "config", "user.name", "repo-view MCP test")
	writeFixtureFile(t, root, `package demo
...
func connect(t *testing.T, server *mcp.Server) (*mcp.ClientSession, func()) {
	...
	client := mcp.NewClient(
		&mcp.Implementation{Name: "repo-view-test", Version: "v1"},
