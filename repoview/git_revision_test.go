...
if [ "$flip" -eq 1 ] && [ ! -e "$REPO_VIEW_TEST_FLIP_STATE" ]; then
  "$REPO_VIEW_TEST_REAL_GIT" "$@"
  status=$?
  if [ "$status" -eq 0 ]; then
    : > "$REPO_VIEW_TEST_FLIP_STATE"
    "$REPO_VIEW_TEST_REAL_GIT" update-ref HEAD "$REPO_VIEW_TEST_FLIP_COMMIT" >/dev/null 2>&1 || exit 97
  fi
  exit "$status"
fi
exec "$REPO_VIEW_TEST_REAL_GIT" "$@"
`
	if err := os.WriteFile(wrapperPath, []byte(wrapper), 0o700); err != nil {
		t.Fatal(err)
	}
	statePath := filepath.Join(wrapperRoot, "flipped")
	t.Setenv("REPO_VIEW_TEST_REAL_GIT", realGit)
	t.Setenv("REPO_VIEW_TEST_FLIP_COMMIT", headC)
	t.Setenv("REPO_VIEW_TEST_FLIP_STATE", statePath)
...
func TestChangedBaseReadsCommittedHeadSnapshot(t *testing.T) {
	...
	base := view.gitOutput("rev-parse", "HEAD")
	writeFile(t, root, "found.go", "package demo\n\nfunc committed() {}\n")
	runGit(t, root, "add", "found.go")
	runGit(t, root, "commit", "-m", "head")
	head := view.gitOutput("rev-parse", "HEAD")
