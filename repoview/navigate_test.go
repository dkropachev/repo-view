...
func TestChangedReportsEveryUntrackedSourceLine(t *testing.T) {
	...
	view := mustView(t, root)
	if changed, err := view.changedLines("new.go", "", view.gitOutput("rev-parse", "HEAD"), 5); err != nil {
		t.Fatal(err)
...
func TestChangedLineDetectionStreamsOversizedDeletedLines(t *testing.T) {
	...
	changed, err := view.changedLines(
		"found.go", "", view.gitOutput("rev-parse", "HEAD"), 1,
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestChangedUntrackedSnapshotUsesLinkedWorktreeAttributes(t *testing.T) {
	...
	runGit(t, mainRoot, "config", "user.email", "repo-view@example.test")
	runGit(t, mainRoot, "config", "user.name", "repo-view test")
