...
- Unknown repository path or exact source identifier: scopesifter find <QUERY>... --root . --match auto --include both --return locations --context {{navigation_context_cap}} --limit {{changed_limit}} --max-code-lines {{changed_max_code_lines}} --max-patch-lines {{changed_max_patch_lines}} --json. The response reports matched_as and each result reports finding as file, symbol, or other; inspect only the selected location or outline only the selected file.
...
For <BASE>, use the revision stated by the task; do not substitute another revision. {{change_navigation_instructions}} For non-change tasks, batch queries that answer the same question and do not raise the listed limits. Do not call collaboration, subagent, spawn-agent, or agent-wait tools. Do not read or invoke Codex skills, plugins, hooks, or marketplace resources; they are outside this navigation task and invalidate the run.
